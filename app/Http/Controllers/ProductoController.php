...
public function delete(Request $request)
{
    // dd($request->all());
    // Obtener los datos del formulario
    $producto_id = $request->input('idproducto_Del');


    // Llamar al procedimiento almacenado para actualizar el producto
    \DB::statement("CALL sp_delete_producto(?)",
        [$producto_id]
    );

    // Redirigir a la vista con un mensaje de éxito

    $language = 'es'; // Reemplaza 'es' con el valor correcto
    return redirect()->route('ecommerce.products', ['language' => $language])->with('success', 'Producto actualizado exitosamente.');
